...
bool uv_from_corners_with_height (vec3 a, vec3 b, vec3 c, vec3 d, float h, vec3 p, out vec3 uv) {
	...
	layout(location = 5) in vec4 inst_uv_range;
	layout(location = 6) in vec2 inst_size;
	...
	void main () {
		vec3 bez_grad;
		v.pos = bezier(inst_bez_a, inst_bez_b, inst_bez_c, inst_bez_d, mesh_t, bez_grad);
		
		v.right = normalize(vec3(bez_grad.y, -bez_grad.x, bez_grad.z)) * inst_size.x * 0.5;
		v.t = mesh_t;
		v.height = inst_size.y;
		
		v.uv_range = inst_uv_range;
		v.col = inst_col;
	}
#endif
	...
	*/
	...
	void main () {
		...
		vec3 local_coords;
		if (!uv_from_corners_with_height(g.a, g.b, g.c, g.d, g.height, gbuf_pos_world, local_coords))
			discard;
		
		// map current block into correct bezier t, and use that as uv
		...
		frag_pbr   = vec4(rough,0,0, rough_alpha); // blended substractively
	}
#endif
